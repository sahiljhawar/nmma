        ...
        for filt in self.filters:
            ...
            if "em_syserr" in self.parameters:
                ...
                if filt=="K":
                    ...
                    data_sigma = np.sqrt(data_sigma**2 + em_syserr_all(data_time) ** 2)
            ...
            if len(infIdx) > 0:
                if "em_syserr" in self.parameters:
                    ...
                    if filt=="u":
                        gausslogsf = scipy.stats.norm.logsf(data_mag[infIdx], mag_est[infIdx], em_syserr_u(data_time[infIdx]))
                    if filt=="K":
                        gausslogsf = scipy.stats.norm.logsf(data_mag[infIdx], mag_est[infIdx], em_syserr_K(data_time[infIdx]))
                    else:
                        gausslogsf = scipy.stats.norm.logsf(data_mag[infIdx], mag_est[infIdx], em_syserr_all(data_time[infIdx]))
